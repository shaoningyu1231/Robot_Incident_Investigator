...
场景配置放在 `scenarios/*.json`。配置只保存可公开的合成数值和预期判别结果；信号分段逻辑仍在 `tools/scenario.py`。

当前已纳入 eval 的场景：

| 场景 | 假设 | 预期 | 目的 |
|---|---|---|---|
| `obstacle_stop` | `concl_obstacle_stop` | `high/ok` | confirmed obstacle stop |
| `planned_stop` | `concl_obstacle_stop` | `low/ok` | true-negative,避免把计划内停车误报成事故 |
| `sensor_disagreement` | `concl_obstacle_stop` | `low/conflicting` | 传感器矛盾时给 insufficient evidence,避免硬编根因 |

`tools/eval_scenarios.py` 会枚举这些配置,逐个执行:

```text
generate synthetic bag → export incident assets → shared rules score hypothesis → compare expected
```

eval 产物写入 `eval_build/` 和 `eval_incidents/`,两者均为确定性重建产物且已 gitignore,不入库。现场 demo 仍只读 `incident/`,不做场景切换。

同一 hero 上的恢复窗口也进入 eval:

- `[12,18] → blocked`
- `[19,25] → conditions_met`

这证明 recovery 会随障碍清除从 blocked 变为 conditions_met,但它仍是恢复条件检查,不是安全认证。
...
- 多场景判别 `tools/eval_scenarios.py` 已跑通:`3/3 scenarios PASS + 2/2 recovery windows PASS`。
